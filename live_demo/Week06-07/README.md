...
[EDIT] The fruits and vegs will stand upstraight as shown in the photo when placed in the arena, e.g., the garlic won't be lying on its side.
...
- [EDIT] after running operate.py to gather the set of observations and their matching robot pose and the SLAM map, run TargetPoseEst.py in a new terminal to generate targets.txt (map of objects) from these observations.
...
[EDIT] This example detector was trained using a total of 1000 images (around 100 manually annotated), with 4 photos taken by the robot on each of the three types of targets from different viewing angles and the rest of the image generated by randomising the background.
